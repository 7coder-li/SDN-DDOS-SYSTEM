...
## 十二、README merge conflict 处理（你刚提到的问题）
...
2. 若检测到冲突，打开 `README.md`，删除冲突标记并保留你需要的内容。
3. 处理完成后再次运行检测脚本，确认输出为“未检测到”。
4. 最后提交：
...
> 本仓库新增了 `.gitattributes`：`README.md merge=union`，用于降低 README 这类持续追加文档在合并时冲突概率。
